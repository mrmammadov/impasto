...
export const knife = {
  ...
  render(ctx, stroke, { rng, params, flow }) {
    ...
    if (P.length === 1) {
      ...
      const dx = Math.cos(a) * rad * 0.7, dy = Math.sin(a) * rad * 0.7;
    ...
    const right = P.map((p, i) => [p[0] - normals[i][0] * half * (0.8 + 0.2 * (i / (n - 1))), p[1] - normals[i][1] * half * (0.8 + 0.2 * (i / (n - 1)))]);

    const v = 1 + gauss(rng) * 0.03, warm = gauss(rng) * 0.01;
    ...
    if (rng() < 0.2 + 0.6 * params.dry && n >= 2) {
      ...
      for (let s = 0; s < streaks; s++) {
        ...
        const nx = normals[n - 1][0], ny = normals[n - 1][1];
        const x0 = a[0] + (b[0] - a[0]) * t0 + nx * off, y0 = a[1] + (b[1] - a[1]) * t0 + ny * off;
        const x1 = b[0] + nx * off, y1 = b[1] + ny * off;
